...
{
	VelodyneLaserScan::VelodyneLaserScan(ros::NodeHandle &nh) : ring_count_(0), nh_(nh) /*,srv_(nh)*/
	{
		nh_.param("ring", cfg_.ring, -1);
		nh_.param("resolution", cfg_.resolution, 0.007);
		ros::SubscriberStatusCallback connect_cb = boost::bind(&VelodyneLaserScan::connectCb, this);
		pub_ = nh_.advertise<sensor_msgs::LaserScan>("scan", 10, connect_cb, connect_cb);
	...
	void VelodyneLaserScan::recvCallback(const sensor_msgs::PointCloud2ConstPtr& msg)
	{
		...
		if((offset_x >= 0) && (offset_y >= 0) && (offset_r >= 0))
		{
			...
			if((offset_x == 0) && (offset_y == 4) && (offset_z == 8) && (offset_i == 16) && (offset_r == 20))
			{
				...
				{
					const uint16_t r = *((const uint16_t*)(&it[5]));
					if(r == ring)
					{
						const float x = it[0];
						const float y = it[1];
