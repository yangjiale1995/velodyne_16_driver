...
namespace velodyne_driver
{
  static const size_t packet_size =
    sizeof(velodyne::VelodynePacket().data);
   ...
   */
  Input::Input(ros::NodeHandle &node, uint16_t port):
    private_nh_(node),
    port_(port)
  {
    private_nh_.param("device_ip", devip_str_, std::string("192.168.1.200"));
  ...
  int InputSocket::getPacket(velodyne::VelodynePacket *pkt, const double time_offset)
  {
  ...
  /** @brief Get one velodyne packet. */
  int InputPCAP::getPacket(velodyne::VelodynePacket *pkt, const double time_offset)
  {
